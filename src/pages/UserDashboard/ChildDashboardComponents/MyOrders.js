                            ...
                            <Row className={cx('view')}>
                                <Col className={cx('view-col')} xs={12} md={8}>
                                    ...
                                    {getOrderItemByOrderId(ele.id).map(prod => {
                                        var isFeedbackBefore = prod.prodcut.feedbacks.find(feed => feed.orderId == prod.orderId && feed.productId == prod.prodcutId)
                                        return (
                                        <div key={prod.id} className="grid grid-cols-5 gap-4">
                                            ...
                                            <div>{isFeedbackBefore ? 
                                                        <span onClick={() => viewFeedback(isFeedbackBefore)} className={cx('order-row-body-view')}>View Feedback</span> 
                                                        : <span onClick={() => feedback(prod.orderId,prod.prodcutId)} className={cx('order-row-body-view')}>Feedback</span>}
                                            </div>
                                        </div>
                                    )})}
                                </Col>
      ...
      <>
        {isLoading && <LoadingSpinner />}
          ...
          >
            ...
            <Modal.Footer>
              <ButtonBootstrap onClick={props.onHide}>Close</ButtonBootstrap>
              <ButtonBootstrap variant="primary" onClick={sendFeedback}>Save</ButtonBootstrap>
            </Modal.Footer>
          </Modal>
      </>
    );
