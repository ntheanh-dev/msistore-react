...
function FilterNav({ filter, setFilter }) {
    ...
    useEffect(() => {
        const fetchAPI = async () => {
            try {
                const res = await API.get(endpointsV2['category']);
                console.log(res.data.resutls);
                setCate(res.data.resutls);
            } catch (e) {
                console.log(e);
            }
        };
    ...
    return (
        <>
            <div className={cx('background')}>
                ...
                {!isMobile && (
                    <Button outline onClick={handleClearFilter}>
                        Clear Filter
                    </Button>
                )}
                ...
                {toogleList.price &&
                    priceList.map((ele) => (
                        <option key={ele.id} onClick={(e) => handleSetFilter(e, 'price')}>
                            {ele.title}
                        </option>
                    ))}
                ...
                <Button primary onClick={handleApplyFilter}>
                    Apply filters {getNumFilter(tempFilter) > 0 && `( ${getNumFilter(tempFilter)} )`}{' '}
                </Button>
            </div>
            {!isMobile && (
                <>
                    ...
                    <Row>
                        {brands.map((brand, index) => (
                            <Col md={6} key={index} className={cx('brand')}>
                                <img src={brand.name} alt="alt" />
                            </Col>
                        ))}
                    </Row>
                    <div className={cx('background')}>
