...
function Product({ data, quanti, primary, isHorver, isInCart, isSerachResult, isCheckout }) {
        ...
        <>
                ...
                <div className={cx('product')} onClick={() => handleClickItem(data.id)}>
                    <>
                        ...
                        <div className={cx('reviews')}>
                            ...
                            <div className={cx('review-quanti')}>Reviews ({20})</div>
                        </div>
                    ...
                    <div className={cx('layout-hover')}>
                        ...
                        <Button onClick={(e) => handleAddToCart(data, e)} outline lefticon={<AiOutlineShoppingCart />}>
                            Add to cart
                        </Button>
                    </div>
                </div>
            ...
            {isHorver && (
                <div className={cx('wrapper-isHover')} onClick={() => handleClickItem(data.id)}>
                    <div className={cx('quanti-isHover')}>
                        {data.cartQuantity}
                        <span>x</span>
                    </div>
                ...
                <div className={cx('wrapper-isSearch')} onClick={() => handleClickItem(data.id)}>
                    ...
                    <div className={cx('content')}>
                        ...
                        <div className={cx('right-isSearch')}>
                            <div className={cx('name-isSearch')}>{data.name}</div>
                            <div className={cx('title-isSearch')}>{data.description}</div>
                            <div className={cx('prices-isSearch')}>
                                <div className={cx('old')}>{Formatter.format(data.oldPrice)}</div>
                                <div className={cx('new')}>{Formatter.format(data.newPrice)}</div>
                            </div>
                            <Button onClick={(e) => handleAddToCart(data, e)} primary>
                                Add to cart
                            </Button>
                        </div>
                    </div>
